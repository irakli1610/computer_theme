...
public final class Laptop extends Computer implements  IFold{
	//----fields
		...
		public Laptop() {}
		public Laptop(CPU cpu, FAN fan, GPU gpu, Keyboard keyboard, Memory memory, Monitor monitor,
				MotherBoard motherBoard, PowerSupply pow, RAM ram,  TouchPad touchPad, Speaker speaker,User user) {
			...
			try {
				if(min<0||sec<0) {
					...
					if(isRestartApps==true) {
						LOGGER.info("system in your laptop will reboot after "+min + " minute and "+ sec + " second and apps will be restarted");
					}else {
						LOGGER.info("system in your laptop will reboot after "+min + " minute and "+ sec + " second and apps won't be restarted");
					}
				}
			}catch(NegativeInputException e){
				LOGGER.error("you entered negative minute or second, please enter positive numbers");
